...
public class RazerTrap : PlacedTrap
{
    ...
    public override void Spawn(List<Vector2Int> region, LevelGenerator levelGenerator)
    {
        ...
        for (int posIdx = 0; posIdx < region.Count; ++posIdx)
        {
            ...
            {
                GameObject trapObj = levelGenerator.PlaceMapPrefab("RazerTrap", pos.x, pos.y);
                RazerTrap razers = trapObj.GetComponent<RazerTrap>();

                if (!uniform)
                    razers.timeOffset = 0.25f * spikeNum * direction;
